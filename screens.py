...
class StartScreen(Screen):
    def __init__(self, background):
        ...
        self.enterText = self.font.render(
        ...
        self.menuRect = (
            50,
            50,
            self.background.res[0] - 100,
            self.background.res[1] - 100,
        )

        # TODO: for now, adding another rect for the rectangle so that it is slightly bigger than text
        self.nameRect1 = (
            40 + self.background.res[0] // 5,
            40 + self.background.res[1] // 5,
            self.background.res[0] // 2,
            self.font.size("1")[1] + 20,
        )
        self.nameRect = (
            50 + self.background.res[0] // 5,
            50 + self.background.res[1] // 5,
            self.background.res[0] // 2,
            self.font.size("1")[1],
        )
        ...
        self.structRectList = []
        self.configSelectText = self.font.render(
            "Choose Config", True, (0, 0, 0), (255, 255, 255)
        )
        self.configSelectRect = (
            40 + self.background.res[0] // 5,
            40 + self.background.res[1] // 5,
            self.background.res[0] // 2,
            self.font.size("1")[1] + 20,
        ...
        for i in range(9):
            self.structRectList.append(
                (
                    40 + self.background.res[0] // 5,
                    self.background.res[1] // 3 + 60 * i,
                    self.background.res[0] // 2,
                    self.font.size("1")[1],
                )
            )
            self.structNameList.append(
                self.font.render(f"Struct {i}", True, (0, 0, 0), (255, 255, 255))
            )
        ...
        if self.configSelect:
            ...
            for i in range(len(self.structRectList)):
                self.background.screen.blit(
                    self.structNameList[i], self.structRectList[i]
                )
    ...
    def hover(self, mousePos):
        ...
        for rect in self.structRectList:
            ...
            inY = y in yRange
            if inX and inY:
                self.structNameList[i] = self.font.render(
                    f"Struct {i}", True, (0, 255, 0), (255, 255, 255)
                )
            else:
                self.structNameList[i] = self.font.render(
                    f"Struct {i}", True, (0, 0, 0), (255, 255, 255)
                )
            i += 1
...
class PauseScreen(Screen):
    def __init__(
        self, level, levels, round, rounds, background, config, agents=[], levelStart=0
    ):
        """
        ...
        self.nextRect = (
            600,
            self.background.res[1] - 50 - self.font.size("Next")[1],
        ) + self.font.size("Next")
        ...
        self.startRect = (
            self.background.res[0] * 3 // 4,
            self.background.res[1] * 9 // 10,
            self.font.size("Start")[0] + 20,
            self.font.size("Start")[1] + 20,
        )
        self.nextRoundRect = (
            460,
            550,
            self.font.size("Next Round")[0] + 10,
            self.font.size("Next Round")[1] + 10,
        )

        self.startTextRect = (
            self.background.res[0] * 3 // 4 + 5,
            self.background.res[1] * 9 // 10 + 5,
            self.font.size("Start")[0] + 20,
            self.font.size("Start")[1] + 20,
        )
        self.nextRoundTextRect = (
            465,
            555,
            self.font.size("Next Round")[0] + 10,
            self.font.size("Next Round")[1] + 10,
    ...
    def renderQuestions(self):
        ...
        yOff = 0
        for q in self.config["questions"]:
            qRendered = self.font.render(
                f"{yOff + 1}. {q['question']}", True, (0, 0, 0)
            )
            ...
            if qType < 2:
                for a in q["answers"]:
                    ...
                    aRenderedRect = (
                        xOff + 8,
                        yC + qRendered.get_height() + 8,
                        self.font.size(a)[0] + 8,
                        self.font.size(a)[1] + 8,
                    )
                ...
                answers.append(
                    (
                        (
                            self.background.res[0] // 4 + 26 + incr * currVal,
                            y + qRendered.get_height() * 1.5,
                        ),
                        qRendered.get_height() / 2,
                        (
                            self.background.res[0] // 4 + 15,
                            self.background.res[0] // 4 + 511,
                        ),
                        currVal,
                        currValRender,
                        False,
    ...
    def blitSliderAnswer(self, qTup, aa):
        ...
        for i in range(11):
            ...
            pygame.draw.rect(
                ...
                (0, 0, 0),
                (
                    x + offset + 20 + self.font.size(f"{i}")[0] // 2,
                    y + q.get_height() * 2 - 10,
                    1,
                    10,
                ),
            ...
            self.background.screen.blit(
                nRendered, (x + offset + 21, y + q.get_height() * 2, 500, 500)
            )
        ...
        pygame.draw.rect(
            self.background.screen,
            (0, 53, 148),
            (x + 10, y + q.get_height(), cX - (x + 10), lenY),
            0,
            2,
        )
    ...
    def blitTextAnswer(self, qTup, aa):
        ...
        pygame.draw.rect(
            self.background.screen,
            (0, 0, 0),
            (x + 10, y + q.get_height() + 25, 520, lenY + 10),
            1,
            2,
        )
        self.background.screen.blit(self.nameText, self.nameRect)
    ...
    def blitSumStats(self):
        ...
        pygame.draw.rect(
            self.background.screen, (200, 200, 200), (250, 250, 800, 400), 0, 3
        )
    ...
    def blitFinalStats(self, cList):
        ...
        self.background.screen.fill((255, 255, 255))
        levelTxt = self.font.render(
            f"Level {self.level + 1} finished!", True, (0, 0, 0)
        )
        self.background.screen.blit(
            levelTxt, (0, 0, self.background.res[0], self.background.res[1])
        )
    ...
    def hoverMenuText(self, mousePos):
        ...
        # next round
        inX = x in range(
            self.nextRoundRect[0], self.nextRoundRect[0] + self.nextRoundRect[2]
        )
        inY = y in range(
            self.nextRoundRect[1], self.nextRoundRect[1] + self.nextRoundRect[3]
        )
        if inX and inY:
                    ...
                    if inX and inY:
                        ...
                        if qType == 0:
                            ...
                            for aTxt, aRender, aRect, c in aList:
                                ...
                                self.aTextList[currInd1][aListIndex] = (
                                    aTxt,
                                    newR,
                                    aRect,
                                    False,
                        ...
                        self.aTextList[currInd1][currInd2] = (
                            ansTxt,
                            nAnsRender,
                            ansRect,
                            newChoice,
        ...
        for currInd1, aList in enumerate(self.aTextList):
            ...
            if qType == 2:
                ...
                if inY and (self.selected < 0 or self.selected == currInd1):
                    ...
                    # rounding lol
                    if currVal - int(currVal) >= 0.5:
                        currVal = math.ceil(currVal)
                    elif currVal - int(currVal) < 0.5:
                        currVal = int(currVal)
...
class InstrScreen(Screen):
    def __init__(self, res):
        ...
        self.startRect = (
            1100,
            710,
            self.font.size("Start")[0] + 10,
            self.font.size("Start")[1] + 10,
        )
        self.startTextRect = (
            1100 + 5,
            710 + 5,
